...
function formatMessageText(text: string) {
  if (!text) return text;
  const regex = /(?:https?:\/\/)?wa\.me\/905546141492/gi;
    ...
    <>
      {parts.map((part, i) => (
        <span key={i}>
          {part}
          {i !== parts.length - 1 && (
            <a href="https://wa.me/905546141492" target="_blank" rel="noopener noreferrer"
               style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', 
               background: '#25D366', color: 'white', padding: '8px 16px', 
               borderRadius: '20px', textDecoration: 'none', fontSize: '13px', marginTop: '8px' }}>
